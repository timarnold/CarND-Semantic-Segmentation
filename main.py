...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # check out decoder lesson to add skip layers
    ...
    output_3 = tf.layers.conv2d_transpose(
        vgg_layer3_out, num_classes, 4, 2, padding='same',
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)
    )
    output_4 = tf.layers.conv2d_transpose(
        vgg_layer4_out, num_classes, 16, 8, padding='same',
        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)
    )
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            ...
            tensorflow_X = tf.placeholder(tf.float32, (None, 160, 575, 3))
            tensorflow_y = tf.placeholder(tf.float32, (None, 2))
            feed_dict = {
                tensorflow_X: image,
                tensorflow_y: label,
                keep_prob: 1.0
